...
interface McUiCardProps {
  product: McUiProduct;
}

export function McUiCard({ product }: McUiCardProps) {
  ...
  return (
    <Card className="relative overflow-hidden bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 transition-all duration-300 hover:shadow-xl hover:scale-[1.03] hover:-translate-y-1">
      {badgeText && (
        <Badge className="absolute top-4 right-4 z-10 bg-purple-500 hover:bg-purple-600 transition-all duration-200">
          {badgeText}
        </Badge>
      )}
